param(
    ...
    [string[]]$SiteRecoveryTimes = @("08:30", "09:30", "10:30")
)
...
foreach ($recoveryTime in $SiteRecoveryTimes) {
    if ([string]::IsNullOrWhiteSpace($recoveryTime)) {
        continue
    }
    $taskSuffix = $recoveryTime.Replace(":", "")
    Register-AIRadarTask -Name "Site Recovery $taskSuffix" -TaskName "recover-site" -TriggerTime $recoveryTime
}
